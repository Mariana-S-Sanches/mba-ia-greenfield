...
- **Background Processing:** Heavy tasks (like Video Processing) use `BullMQ` for message queues and workers. The `VideosProcessor` handles `ffmpeg` operations.
- **Storage:** Uses `@aws-sdk/client-s3` for interacting with MinIO Object Storage, generating presigned URLs for client-side uploads.
